...
const ModifierSchema = z.object({
  id: z.number(),
  type: z.enum(['LEFT', 'RIGHT', 'FLIP', 'CLONE']),
  name: z.string().describe('The name of the modifier, which should reflect its type.'),
  description: z.string().describe('A brief (1-sentence) description of the modifier action.'),
});
...
const GenerateCardsInputSchema = z.object({
  ...
  existingModifiers: z.array(ModifierSchema).describe('An array of existing modifiers to use as a structural template.'),
});
...
Your task is to generate a new set of Rules, Prompts, and Modifiers based on a user-provided theme: "{{theme}}".

You must follow these instructions carefully:
1.  **Maintain Structure:** Use the provided existing cards as a template for the structure and quantity of new cards. The number of new rule groups, prompts, and modifiers should be the same as the number of existing ones.
2.  **Preserve Mechanics:** The core mechanic of each card, especially modifiers, must be preserved.
    *   A 'FLIP' modifier must still be about flipping a rule.
    *   'LEFT' and 'RIGHT' modifiers must still involve passing rules to other players.
    *   'CLONE' must still involve duplicating a rule.
    *   Flipped rules should be the thematic opposite of their primary rule.
3.  **Inject Theme:** Creatively infuse the "{{theme}}" into the 'name' and 'description' of each card. Be clever and funny.
...
- Modifiers: {{{json existingModifiers}}}

Now, generate a full new set of themed content. Your response MUST be a valid JSON object matching the output schema.
`,
});
